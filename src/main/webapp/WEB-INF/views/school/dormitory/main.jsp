...
<body>
	...
	<security:authorize url="">
		<script type="text/javascript">
			$('#dg').datagrid({
				//数据源
				url : '${prePath}/school/dormitory/getPage',
				...
				onDblClickRow : function(index,row){//双击一行查看详细信息
					...
					showDialog("win","${prePath}/school/dormitory/detail","查看宿舍详情页面");
				}, 
				//菜单栏
				toolbar : [ {
					iconCls : 'icon-add',
					text : '增加',
					handler : function() {
						showDialog("win","${prePath}/school/dormitory/add","添加宿舍页面");
					}
				}, '-', {
					iconCls : 'icon-edit',
					text : '修改',
					handler : function() {
						...
						showDialog("win","${prePath}/school/dormitory/edit","修改宿舍页面");
					}
				}, '-', {
					iconCls : 'icon-remove',
					text : '删除',
					handler : function() {
						...
						$.messager.confirm('操作提示', '这是一个不可逆的操作,您确定要删除这些数据吗？', function(r){
							...
							$.ajax({
								type : "POST",//请求方式
								dataType : 'json',
								url : "${prePath}/school/dormitory/delete",//请求目的URL
					...
					handler : function() {
						...
						$.messager.confirm('操作提示', '您确定要导出'+selections.length+'条数据吗？', function(r){
							...
							form.attr("action","${prePath}/school/dormitory/exportSelections");
							$.each(selections,function(rowNum,row){
								//定义input标签
								var input = $("<input>");
					...
					handler : function() {
						...
						form.attr("target","");  
						form.attr("method","post");  
						form.attr("action","${prePath}/school/dormitory/exportAll");
						//end form
						$("#win").append(form);//将表单放置在web中  
						form.submit();//表单提交 	
					}
	...
	<!-- user可接受内容 -->
	<security:authorize url="" access="not hasAuthority(T(com.lys.zhku.utils.Authorities).admin.auth)">
