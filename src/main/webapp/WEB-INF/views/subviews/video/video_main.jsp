		...
		$('#dg').datagrid({
			...
			toolbar : [ {
				...
				handler : function() {
					...
					$.messager.confirm('操作提示', '这是一个不可逆的操作,您确定要删除这些数据吗？', function(r){
						...
						$.ajax({
							type : "POST",//请求方式
							dataType : 'json',
							url : "${prePath}/video/delete",//请求目的URL
				...
				handler : function() {
					...
					form.attr("action","${prePath}/video/download");
					//end form
					//定义input标签
					var input=$("<input>");  
				...
				handler: function(){
					...
					var row = selections[0];
					showDialog("win","<c:url value='/subviews/video/playVideo'/>","视频播放");
				}
			} ],
			//列    
			columns : [ [ {
